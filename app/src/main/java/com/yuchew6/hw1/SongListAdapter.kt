...
class SongListAdapter(private var listOfSong: List<Song>): RecyclerView.Adapter<SongListAdapter.SongViewHolder>() {
    ...
    var onSongClickListener: ((song: Song, theIndex: Int) -> Unit)? = null
    ...
    override fun onBindViewHolder(holder: SongViewHolder, position: Int) {
        ...
        holder.bind(artistName, songTitle, songPic, listOfSong[position], position)
    }

    fun change(newSong: List<Song>) {
        val callback =
            SongDiffCallback(listOfSong, newSong)
    ...
    inner class SongViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(name: String, title: String, pic: Int, song: Song, index: Int) {
            ...
            itemView.setOnClickListener {
                // update song and index
                onSongClickListener?.invoke(song, index)
            }
